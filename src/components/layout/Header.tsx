    ...
    <header className="h-16 border-b border-border bg-blue-600 text-white shadow-sm">
      <div className="flex h-full items-center justify-between px-4 gap-2 sm:gap-4">
        ...
        <div className="flex items-center gap-1 sm:gap-2 shrink-0">
            ...
            <DropdownMenuContent align="end" className="w-80 bg-white text-black">
              ...
              <div className="max-h-80 overflow-y-auto">
                {notifications.map((notification, index) => (
                  <DropdownMenuItem key={index} className="flex flex-col items-start p-3 space-y-1 hover:bg-muted/30">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" className="flex items-center gap-2 px-2 py-2 h-10 hover:bg-blue-700 text-white">
                <div className="w-8 h-8 rounded-full bg-white/20 text-white flex items-center justify-center">
                  <UserCircle size={24} weight="fill" />
                </div>
                {/* Hide name on small screens */}
                <div className="hidden md:block text-left">
                  <div className="text-sm font-semibold truncate max-w-[100px]">Bigfoot</div>
